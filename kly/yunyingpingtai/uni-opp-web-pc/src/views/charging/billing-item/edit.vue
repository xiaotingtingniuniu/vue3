...
const getInfoedit = async () => {
    const params = {
        ...
        appKey: 'A1001001',
        busiCode: 'A1001001',
        configCode: 'UC32502950655598592',
        name: '查询计费项详情接口',
        url: '/settlement/feeConfig/detail',
        source: '结算中台'
    }
    const { code, data } = await getMerchantListCode(params)
    console.log(code)
    if (Number(code) === 200) {
        ruleForm.value.id = data?.id ?? ''
        ruleForm.value.feeMode = data?.feeMode ?? ''
        ruleForm.value.feeScene = data?.feeScene ?? ''
        ruleForm.value.feeCode = data?.feeCode ?? ''
        ruleForm.value.ebillingitem = data?.ebillingitem ?? ''
        ruleForm.value.startTime = data?.startTime ?? ''
        ruleForm.value.endTime = data?.endTime ?? ''
        ruleForm.value.feeAttribution = data?.feeAttribution ?? ''
        ruleForm.value.feeAmount = data?.feeAmount / 100 ?? ''
    }
}
getInfoedit()
