                ...
                >
                    <div class="search-input">
                        ...
                        <!-- <el-form-item label="业务类型" prop="busiType">
                            <el-select
                                v-model="searchData.busiType"
                                clearable
                                placeholder="请选择"
                            >
                                <el-option label="团长分销" :value="1" />
                                <el-option label="货柜主" :value="2" />
                                <el-option label="批发商" :value="3" />
                                <el-option label="代运营商" :value="4" />
                                <el-option label="分账人" :value="5" />
                                <el-option label="本地生活" :value="6" />
                            </el-select>
                        </el-form-item> -->
...
import { businessTHead } from '../tHead'
import { getWarterList, exportDetails } from '@/api/accountManagement/index'
import SystemList from '@/composables/TablePagination/index.vue'
import { ElMessage } from 'element-plus'
const formRef = ref()
const recordDirectionKeyValue = ref({ 1: '入账', 2: '出账' })
// const busiTypeKeyValue = ['货柜主', '批发商', '代运营商']
// 1-团长分销,2-货柜主,3-批发商,4-代运营商,5-分账人
const businessKeyValue = ref({ 1: '团长分销', 2: '货柜主', 3: '批发商', 4: '代运营商', 5: '分账人' })
...
const getList = async () => {
    ...
    const res = await getWarterList(param)
    if (res.code === '200') {
...
const rules = reactive({
    ...
    applyTime: [
        { required: true, message: '请输入动账时间', trigger: 'change' }
    ]
})
