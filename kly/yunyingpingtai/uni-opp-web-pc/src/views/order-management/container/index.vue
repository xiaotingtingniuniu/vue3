<template>
    <div>
        <el-form ref="resetFormData" :model="form" label-width="120px" :inline="true">
            ...
            <el-form-item label="开门记录" prop="applyCode">
                <el-input v-model="form.applyCode" placeholder="请输入开门记录" />
            </el-form-item>
            ...
            <el-form-item label="合同货柜编号" prop="poiId">
                <el-input v-model="form.poiId" placeholder="请输入合同货柜编号" />
            </el-form-item>
...
const tHead = [
    ...
    { align: 'center', tooltip: true, prop: 'applyCode', label: '开门记录', minWidth: 180 },
    // { align: 'center', tooltip: true, prop: 'deliveryMethod', label: '配送方式', minWidth: 130, slot: true },
    { align: 'center', tooltip: true, prop: 'poiId', label: '合同货柜编号', minWidth: 180 },
    { align: 'center', tooltip: true, prop: 'deviceNo', label: '设备ID', minWidth: 180 },
    { align: 'center', tooltip: true, prop: 'deviceBrand', label: '设备品牌', minWidth: 180 },
...
const quotationDetailed = (row) => {
    const routeData = router.resolve({ name: 'container-order-detail', params: { orderNoSub: row.orderNoSub } })
    window.open(routeData.href, '_blank')
    // router.push({ name: 'container-order-detail', params: { orderNoSub: row.orderNoSub } })
}
...
const getOrderList = async () => {
    const params = {
        ...
        appKey: 'A1001001',
        busiCode: 'A1001002',
        configCode: 'UCJH46231323231379456',
        name: '货柜订单列表聚合',
        source: '运营后台'
    }
...
const exportExcel = async () => {
    const params = {
        ...
        // name: '货柜导出查询订单列表',
        // source: '订单中台',
        exportName: '统一运营平台-货柜订单-' + generateDate(),
        configCode: 'UCJH46561113928470528',
        name: '货柜导出订单列表聚合',
        source: '运营后台'
    }
