<template>
    ...
    <div>
        ...
        <el-form ref="resetFormData" :model="searchData" label-width="120px" :inline="true">
            <el-form-item label="二级渠道编号" prop="secondCateCode">
                <el-input v-model="searchData.secondCateCode" placeholder="二级渠道编号" /> </el-form-item>
        ...
        >
            <el-table-column prop="name" label="商品信息" align="left" min-width="150">
                <template v-slot="{row}">
                    <div>
                        ...
                        <span>商品分类：{{row.firstCateName}}/{{row.secondCateName}}/{{row.thirdCateName}}/{{row.fourthCateName}}</span>
                    </div>
                </template>
            </el-table-column>
            <el-table-column
            ...
            <el-table-column prop="address" label="操作" align="center">
                <template v-slot="{row}">
                    <el-button type="text" @click="save(row), dialogVisible = true" v-if="row.skuStatus === 5"> 保存 </el-button>
...
const preservation = async (val) => {
    const params = {
        ...
        body: {
            ...
            busiCode: 'A1001002',
            skuId: getout.skuId,
            channelPrice: Number(getout.purchasePrice),
...
const getRefundList = async () => {
    const params = {
        ...
        name: '查询商品渠道',
        source: '价格中台',
        configCode: 'UC1658206066028BXSH1',
        creatorId: userInfo.value.account,
        modifierId: userInfo.value.account,
        body: {
            appKey: 'A1001000', // 系统编码
