...
<template>
    <div>
        <el-dialog
            ...
            draggable
        >
            <el-form ref="ruleEditFormRef" :model="ruleForm" :rules="rules" label-width="140px" class="demo-ruleForm">
            ...
            <div class="sku-btns">
                ...
                <el-button type="primary" @click="submitRule(ruleEditFormRef)">
                    确认
                </el-button>
            </div>
        </el-dialog>
...
import { repRuleQueryGuizhu, repRuleQueryEquipment, repRuleSave } from '@/api/operate/index'
...
const { ruleForm, ruleTypeStatus, selCitysChecked, isSelOwner, ownerTabData, ownerTabChecked, isSelDevice, deviceTabData, deviceTabChecked } = toRefs(dataState)
...
const init = async () => {
    ...
    if (dataState.ruleForm.ruleType === 1) {
        // 生效城市回显
        if (dataState.ruleForm.ruleDetailsDesc) {
            dataState.ruleForm.ruleDetailsDesc.forEach(item => {
                const catesInfo = [item.provinceCode, item.cityCode]
                dataState.selCitysChecked.push(catesInfo)
            })
        }
    } else if (dataState.ruleForm.ruleType === 2) {
        dataState.ownerTabChecked = dataState.ruleForm.ruleDetailsDesc
    } else if (dataState.ruleForm.ruleType === 3) {
        dataState.deviceTabChecked = dataState.ruleForm.ruleDetailsDesc
    }
}
init()
// 取消
...
// 修改规则提交 02166529039707378T/ 0216634006954912JD  021663241195443O4E 021663124763333VQU
const submitRule = async (formEl) => {
    ...
    await formEl.validate(async (valid, fields) => {
        if (valid) {
            ...
            const res = await repRuleSave(params)
            if (res.code === '200') {
