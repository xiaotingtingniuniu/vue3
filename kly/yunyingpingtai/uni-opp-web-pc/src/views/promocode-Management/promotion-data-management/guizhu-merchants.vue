            ...
            <div class="search-area">
                <el-form ref="formRef" :model="searchData" :inline="true" label-position="right" label-width="100px">
                    ...
                    <el-form-item label="所属城市" prop="cityName">
                        <el-input v-model="searchData.cityName" @keyup.enter="searchList" placeholder="请输入所属城市" clearable />
                    </el-form-item>
                    <el-form-item label="推广产品" prop="productName">
                        <el-input v-model="searchData.productName" @keyup.enter="searchList" placeholder="请输入推广产品" clearable />
                    </el-form-item>
                    ...
                    <el-form-item label="注册手机号" prop="appPhoneMobile">
                        <el-input v-model="searchData.appPhoneMobile" @keyup.enter="searchList" placeholder="请输入注册手机号" clearable />
                    </el-form-item>
                    <el-form-item label="是否首单" prop="isFirst">
                        <el-select v-model="searchData.isFirst" clearable placeholder="请选择" >
                    ...
                    <el-form-item label="交易完成时间" prop="regTime" class="form-item-time">
                    ...
                    <el-form-item label="是否已过退款时效" prop="isRefundEndTime">
                        <el-select v-model="searchData.isRefundEndTime" clearable placeholder="请选择" >
                            <el-option label="请选择" value="" />
                            <el-option label="是" :value="1" />
                            <el-option label="否" :value="0" />
                        </el-select>
            ...
            <div class="tableList-area">
                ...
                >
                    <template #orderStatus="row">
                        {{ row.row.orderStatus === 100 ? '未完成' : row.row.orderStatus === 600 ? '已完成' : row.row.orderStatus === 700 ? '已退款' : row.row.orderStatus === 800 ?'已取消':'-' }}
                    </template>
                    <template #isFirst="row">
                        {{ row.row.isFirst === 1 ? '是' : row.row.isFirst === 0 ? '否':'-' }}
                    </template>
...
import { hzgTheads } from './tHead.js'
...
const getTableList = async (obj, val) => {
    const param = {
        ...
        appKey: 'A1001001',
        busiCode: 'A1001001',
        configCode: 'UC53147068797980672',
        name: '恒生活app推广数据-列表',
        source: '推广系统'

    }

    if (searchData.value.isFirst === '') delete param.body.isFirst
    if (searchData.value.isRefundEndTime === '') delete param.body.isRefundEndTime
    if (searchData.value.orderStatus === '') delete param.body.orderStatus
