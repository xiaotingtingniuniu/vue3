            ...
            <el-row :gutter="24">
                ...
                <el-col :span="8">
                    <el-form-item label="是否创建影子">
                        <el-radio-group disabled v-model="ruleForm.isShadow">
...
const getDetail = async () => {
    ...
    ruleForm.isShadow = data.data[0].isShadow
...
const submitForm = async () => {
    ...
    const goodsDetail = JSON.stringify(goodsObj) === '{}' ? '[]' : JSON.stringify([{ title: '商品介绍说明', index: 1, pics: goodsObj }])
    const params = {
        body: {
            goodsId: props.goodsId,
            appKey: 'A1001000', // 系统编号
            ...
            medias: [
                ...
                // { mediaName: '轮播图', mediaType: 3, mediaUrl: ruleForm.lunbolist[0]?.url || '', id: ruleForm.lunbolist[0].id || '' },
    ...
    const filterMedia = []
    params.body.medias.map((item, index) => {
        if (item.mediaUrl !== '') {
            filterMedia.push(item)
        }
    })
    params.body.medias = filterMedia
