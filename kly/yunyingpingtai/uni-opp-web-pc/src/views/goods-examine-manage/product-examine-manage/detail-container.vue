                ...
                <el-row :gutter="24">
                    ...
                    <el-col :span="8">
                        <el-form-item label="净含量">
                            <el-input v-model="detailInfo.netRatioName" :disabled="disabled" controls-position="right" class="form-action-item-50w"/>
                        </el-form-item>
                    </el-col>
                    <el-col :span="8">
                ...
                <el-row :gutter="24">
                    <el-col :span="12">
                        <el-form-item label="背标图">
                            ...
                            <el-image
                                v-for="img in detailInfo.backscript"
                                :key="img"
                ...
                <el-row :gutter="24">
                    <el-col :span="12">
                        <el-form-item label="模型训练图片">
                            <el-image
                                v-for="img in detailInfo.modelImgs"
                                :key="img"
                                style="width: 100px; height: 100px; margin: 0 20px 20px 0"
                                :src="img"
                                :zoom-rate="1.2"
            ...
            <el-form ref="ruleFormRef" :model="ruleForm" label-position="right" label-width="auto">
                <div>
                    ...
                    <el-form-item label="审批结果" prop="state" :rules="rules.state">
                        <el-radio-group v-model="ruleForm.state" :disabled="!($route.params?.examine === 'examine')" @change="handleClearValidate">
                            <el-radio :label="3">通过</el-radio>
                            <el-radio :label="4">驳回</el-radio>
                        </el-radio-group>
                        <el-button type="primary" style="margin-left: 30px" @click="productDialogShow()" v-if="$route.params?.examine === 'examine'" :disabled="ruleForm.state === 4"> 检索标品库 </el-button>
                    </el-form-item>
                    ...
                    <el-form-item label="是否创建新品" prop="creatProduct" ref="creatProduct" :rules="ruleForm.state !== 4 ? rules.creatProduct : null">
                        <el-radio-group v-model="ruleForm.creatProduct" @change="creatProductChange()" :disabled="!($route.params?.examine === 'examine')|| ruleForm.state === 4">
                        ...
                        <span v-if="ruleForm.creatProduct === 0 && relationSpu"> (标品编码：{{ relationSpu.spuId }}&nbsp;&nbsp;&nbsp;标品名称：{{ relationSpu.spuName }}) </span>
                    </el-form-item>
                    <el-form-item label="销售渠道" prop="saleChannel" ref="saleChannel" :rules="ruleForm.state !== 4 ? rules.saleChannel : null">
                        <el-checkbox-group v-model="ruleForm.saleChannel" :disabled="!($route.params?.examine === 'examine') || ruleForm.state === 4">
                    ...
                    <div class="mt-20">
                        ...
                        <el-button v-if="$route.params?.examine === 'examine'" type="primary" @click="productCheckDialogShow()"> 提交审批结果 </el-button>
                    </div>
                </div>
            </el-form>
        </el-card>
        <el-dialog v-model="dialogVisible" width="50%" :before-close="productCheckDialogHide" title="标品核查">
            <el-form ref="productCheckFormRef" :model="productCheckForm" label-width="120px" :rules="productFormRules">
                <el-row :gutter="24">
                    <el-col :span="12">
                    ...
                    <el-col :span="12">
                        <el-form-item label="商品通用图" prop="StandardList">
                            <cst-upload :limit="1" :multiple="true" v-model:fileList="productCheckForm.StandardList" accept=".png" :mediaType="11" show="hidden" tip="1080*1080，格式：png，大小：小于2M，仅限一张 "></cst-upload>
                            <el-button type="primary" @click="compressionHandle(productCheckForm.StandardList)" :loading="packLoading">一键压缩</el-button>
                        </el-form-item>
                    </el-col>
                    <el-col :span="24">
                        <div style="color:rgba(217, 0, 27, 0.823529411764706)">上传商品通用图后，请点击“一键压缩“按钮，完成货柜Pad图、SPU主图、列表图、缩略图图片补充，再点击”保存并推送模型训练图片“</div>
                    </el-col>
                </el-row>
            </el-form>
            <template #footer>
                <span class="dialog-footer">
                    ...
                    <el-button type="primary" @click="productCheckSubmit(productCheckFormRef)">保存并推送模型训练图片</el-button>
                </span>
            </template>
        </el-dialog>
        ...
        <el-dialog v-model="productDialogVisible" title="关联标品-公共商品库" width="950px" @close="dialogClose">
            <el-form ref="dialogFormRef" :model="dialogSearch" :inline="true" label-position="right" label-width="100px">
                ...
                <el-form-item class="button-box">
                    ...
                    <!-- <el-button type="primary" @click="dialogSearchList" :disabled="dialogSearch.spuIds || dialogSearch.spuName || dialogSearch.barcodes ? false : false">搜索</el-button>
                    <el-button type="primary" @click="dialogResetForm(dialogFormRef)" :disabled="dialogSearch.spuIds || dialogSearch.spuName || dialogSearch.barcodes ? false : false">重置</el-button> -->
                    <!-- <el-button type="primary" @click="bindSpu()" :disabled="dialogSelection ? false : true">关联所选标品</el-button> -->
                </el-form-item>
            </el-form>
            <system-list
            ...
            >
                <template #modelStatus="scope">
                    {{ modelStatusMap[scope.row.modelList?.[0]?.modelStatus] }}
                </template>
                <template #modelMediaUrl="scope">
                    <el-image
                        style="width: 100px; height: 100px"
                        preview-teleported
                        :src="scope.row?.modelMediaList?.find((item) => item.mediaType === 2)?.mediaUrl"
                        :preview-src-list="handleMedialUrl(scope.row.modelMediaList)"
                        fit="cover"
                    />
                </template>
                <template #spuMediaUrl="scope">
                    <el-image
                        style="width: 100px; height: 100px"
                        preview-teleported
                        :src="scope.row?.spuMediaList?.find((item) => item.mediaType === 6)?.mediaUrl"
                        :preview-src-list="handleSpuMediaList(scope.row.spuMediaList)"
                        fit="cover"
                    />
                </template>
            </system-list>
...
const creatProduct = ref(null)
const saleChannel = ref(null)
...
const handleMedialUrl = (data) => {
    const temp = []
    const arr = data?.filter((item) => item.mediaType === 2) || []
    arr.length &&
        arr.forEach((item) => {
            temp.push(item.mediaUrl)
        })

    return temp
}

const handleSpuMediaList = (data) => {
    const temp = []
    const arr = data?.filter((item) => item.mediaType === 6) || []
    arr.length &&
        arr.forEach((item) => {
            temp.push(item.mediaUrl)
        })
...
const productCheckForm = reactive({
    ...
    // listPicUrl: [],
    // suolueList: [], // 缩略图
    // PADImageList: [], // 货柜PAD商品图
    // liebiaoImageList: [], // 列表图
    // zhutuList: [], // zhutuList
    StandardList: [] // 通用图
})
const conmpressImages = reactive({
...
const checkAge = () => {
    console.log(conmpressImages, '--conmpressImages---', conmpressImages?.suolueList[0], '00000000000000')
    if (!productCheckForm?.StandardList?.length ||
     (!conmpressImages?.suolueList?.length) ||
     (!conmpressImages?.PADImageList?.length) ||
     (!conmpressImages?.liebiaoImageList?.length) ||
     (!conmpressImages?.zhutuList?.length)) return ElMessage.error('请点击“一键压缩“按钮，完成图片补充，再点击”保存并推送模型训练图片')
    submitForm()
}
// 表单项规则
...
const productFormRules = reactive({
    ...
    StandardList: [{ required: true, validator: checkAge, trigger: 'change' }]

    // appPicUrl: [{ required: true, message: '请上传', trigger: 'change' }],
    // listPicUrl: [{ required: true, message: '请上传', trigger: 'change' }],
    // StandardList: [{ required: true, message: '请点击“一键压缩“按钮，完成图片补充，再点击”保存并推送模型训练图片“', trigger: 'change' }]
...
const getDetail = async () => {
    ...
    if (res.code === '200') {
        ...
        defaultcode.value = res.data.barcode
        ...
        detailInfo.value.productShelf = res.data.productShelfLife
        detailInfo.value.netRatioName = (res.data.netRatio || '') + (res.data.netRatioUnit || '')
...
const handleClearValidate = (value) => {
    if (value === 4) {
        creatProduct.value.resetField()
        saleChannel.value.resetField()
        ruleFormRef.value.clearValidate()
    }
}
// 基础类目联动
...
const productDialogShow = () => {
    ...
    dialogSearch.value.barcodes = defaultcode.value
    getDialogData()
}
...
const dialogResetForm = (formEl) => {
    if (!formEl) return
    formEl.resetFields()
    dialogSearch.value.barcodes = null
    dialogSelection.value = null
    dialogTableData.value = []
    dialogTotal.value = 0
    dialogCurrentPage.value = 1
...
const dialogTHead = [
    ...
    { align: 'center', tooltip: true, prop: 'saleChannel', label: '销售渠道', minWidth: 180 },
    { align: 'center', tooltip: true, prop: 'barcode', label: '69码', minWidth: 120 },
    { align: 'center', tooltip: true, prop: 'modelStatus', label: '模型训练状态', minWidth: 180, slot: true },
    { align: 'center', tooltip: true, prop: 'spuMediaUrl', label: '标品列表图', minWidth: 150, slot: true },
    { align: 'center', tooltip: true, prop: 'modelMediaUrl', label: '标品模型训练首图', minWidth: 180, slot: true }
]
// 标品库全选操作
...
const productCheckDialogShow = () => {
    ...
    ruleFormRef.value.validate(async (validate) => {
        ...
        if (ruleForm.state === 3 && ruleForm.creatProduct === 1) {
            ...
            productCheckForm.productShelfLife = detailInfo.value.productShelf
...
const submitForm = async () => {
    const spuInfo = {
        ...productCheckForm,
        // medias: [
        //     { mediaName: '通用图', mediaType: 11, mediaUrl: productCheckForm?.StandardList[0]?.url || '' }，
        ...
        // ],
        medias: [conmpressImages?.suolueList[0], conmpressImages?.liebiaoImageList[0], conmpressImages?.PADImageList[0], conmpressImages?.zhutuList[0], conmpressImages?.StandardList[0]],
    ...
    const params = {
        ...
        ...ruleForm,
        // spuId: ruleForm.creatProduct === 1 ? null : null,
    ...
    let PRODUCT_EXAMINE_MANAGE_API = 'PRODUCT_EXAMINE_MANAGE_EXAMINE_TOFETHER_V5'
    if (detailInfo.value.busiCode === 'A1001002') {
        PRODUCT_EXAMINE_MANAGE_API = 'PRODUCT_EXAMINE_MANAGE_EXAMINE_TOFETHER_v4'
    }
    const res = await publicRequest({ key: PRODUCT_EXAMINE_MANAGE_API, body: params })
    if (res.code === '200') {
...
const compressionHandle = async () => {
    if (!productCheckForm?.StandardList?.length) return ElMessage.error('请先上传图片')
    packLoading.value = true
    const param = {
        body: {
            appKey: 'A1001000', // 系统编号
            busiCode: 'A1001002',
            url: productCheckForm.StandardList[0].url // 通用图片URL
        },
        appKey: 'A1001001',
        busiCode: 'A1001001',
        configCode: 'UCJH65442670473641984',
        name: '一键压缩聚合',
        source: '运营后台'
    }
    ...
    if (res.code === '200') {
        ...
        compressImg.value.forEach(el => {
            // 1:短视频、2: 主图 、3: 轮播图、4：缩略图、5：瀑布图、6：列表图，11：商品通用图
            if (el.type === 2) { conmpressImages.zhutuList.push({ mediaUrl: el.url, mediaName: '主图', mediaType: el.type }) }
            if (el.type === 4) { conmpressImages.suolueList.push({ mediaUrl: el.url, mediaName: '缩略图', mediaType: el.type }) }
            if (el.type === 6) { conmpressImages.liebiaoImageList.push({ mediaUrl: el.url, mediaName: '列表图', mediaType: el.type }) }
            if (el.type === 9) { conmpressImages.PADImageList.push({ mediaUrl: el.url, mediaName: '货柜Pad商品图', mediaType: el.type }) }
            if (el.type === 11) { conmpressImages.StandardList.push({ mediaUrl: el.url, mediaName: '商品通用图', mediaType: el.type }) }
        })
    } else {
        packLoading.value = false
        ElMessage({
