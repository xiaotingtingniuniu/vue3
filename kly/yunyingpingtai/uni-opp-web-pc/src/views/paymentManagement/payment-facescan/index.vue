        ...
        <div class="search-area">
            <el-card>
                <el-form ref="formRef" :model="searchData" :inline="true" label-width="150px" label-position="right">
                    ...
                    <el-form-item label="刷脸付状态" prop="wxPayStatus">
                    ...
                    <el-form-item label="支付分单号" prop="thirdPayNo">
                    ...
                    <el-form-item label="创建支付分时间" prop="payTime" class="form-item-time">
                        <el-date-picker
...
import { facelist, refundRecordList, refundRecordStatistics, exportDetails } from '../../../api/paymentManagement/index'
import { facescanThead } from '../tHead'
const payChannelCodeKeyValue = ref({ 1: '微信', 2: '支付宝' })
const refundStatusKeyValue = ref({ TODO: '待创建', CREATED: '商户已创建服务订单', DOING: '服务订单进行中', DONE: '服务订单完成', REVOKED: '商户取消服务订单', USER_PAID: '支付成功' })
...
const getList = async (reset) => {
    ...
    const param = { ...searchData.value, currentPage: pageInfo.value.currentPage, pageSize: pageInfo.value.pageSize, businessCode: 'A1001002', subProductCode: 'A1001002001' }
    if (param.payTime) {
    ...
    const res = await facelist(param)
