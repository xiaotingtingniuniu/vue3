<template>
    ...
    >
        ...
        <template #file="{ file }">
            <!-- <img :src="file.url" /> -->
            <div v-if="(file.response?.data)?(file.response?.data): (file.url)">
                <video v-if="getFileType((file?.response?.data)? (file?.response?.data): (file?.url)) === 'video'" :src="(file?.response?.data)?(file?.response?.data): (file?.url)"></video>
                <img v-if="getFileType((file?.response?.data)?(file?.response?.data):(file?.url)) === 'img'" class="el-upload-list__item-thumbnail" :src="(file?.response?.data)?(file?.response?.data):(file?.url)" alt />

                <span class="el-upload-list__item-actions" v-if="!props.disabled && props.show !=='show'">
                    <span
                ...
                <span class="el-upload-list__item-actions" v-else-if="props.disabled && props.show ==='show'">
                    <span
                        class="el-upload-list__item-preview"
                        @click="handlePictureCardPreview(file)"
                    >
...
import { getupdate } from '@/api/operate/index'
