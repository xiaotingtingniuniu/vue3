        ...
        <div class="coupon-container" v-if="couponList">
            <van-radio-group v-model="checked">
                <template v-for="(item, index) in couponList" :key="index">
                    <div class="coupomn-box" @click="getCheckout(index, item)">
...
onMounted(() => {
    appBridge('setCouponData', {}, (res) => {
        const temp = JSON.parse(res)
        console.log(temp, 'temp')
        couponList.value = temp.couponList
        checked.value = temp.couponList[temp.selectIndex].couponCode
    })
})

// 选中
const getCheckout = (selectIndex, selectCoupon) => {
    appBridge('getSelectCoupon', { selectIndex, selectCoupon })
}
