...
const priceNum = (arg1, arg2) => {
    const m = ref(0)
    const s1 = arg1.toString()
    try {
        m.value = s1.split('.')[1].length
    } catch (error) {}
    return Number(s1.replace('.', '')) * arg2 / Math.pow(10, m.value)
}
...
const couponSelectFn = async () => {
    ...
    for (var i = 0; i < list.length; i++) {
        ...
        Codes.push(objs)
        priceAmount.value = priceAmount.value + (priceNum(list[i].sellPrice, 100) * list[i].quantity)
    }
