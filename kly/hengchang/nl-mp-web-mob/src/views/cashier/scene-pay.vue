...
import { scenePayCashierDoPay } from '@/utils/order'
import { goHomePage2 } from '@/utils/composables'
...
// app自己做回退处理
const appBack = () => appBridge('scenePayCashierBack')
// 安卓物理返回 执行h5PageBack
...
document.addEventListener('visibilitychange', function () {
    if (document.visibilityState === 'visible' && !props.checkOrder && token.value && route.name === 'scene-pay-cashier') {
        cashWindowsDataFn()
    }
})

const orderStatusFn = async () => {
    const { data } = await isSuccessPay({ orderNumber: props.orderNumber })
    if (data === '订单已支付') {
        appBridge('paySuccess', props.orderNumber)
    } else {
        cashWindowsDataFn()
    }
}
...
const goPay = () => {
    if (isApp) {
        appBridge('checkPayChannel', '微信', (payload) => {
            if ((payload) === 'true') {
                scenePayCashierDoPay(props.orderNumber, cashierData.value.payAmount)
