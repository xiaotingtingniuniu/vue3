    ...
    <div class="supplementary-order">
        ...
        <div class="card">
            <div class="order-content">
                <div class="cell">原订单 共{{orderInfo.orderInfo.totalQuantity || 0}}件：调整后 共{{skuCount|| 0}}件</div>
                <div class="cell">原订单金额：{{(orderInfo.orderInfo.totalAmount / 100).toFixed(2)}}元：调整后 {{(skuPrice / 100).toFixed(2)}}元</div>
            ...
            <div class="order-coupon b-t">
                ...
                <div class="number">￥{{(orderInfo.orderInfo.disctAmount / 100).toFixed(2)}}</div>
            </div>
        </div>
                ...
                <van-tab title="修改商品">
                    ...
                    <div class="device-goods-content" v-if="skuList?.length">
                        <div v-for="item in skuList" :key="item.skuId">
                            <goods-item :goods="item" @change="handleChange"/>
                        </div>
                    </div>
                </van-tab>
                <van-tab title="调整金额">
                    <div class="input-container">
                        ...
                        <van-field v-model="price" @blur="onBlur" input-align="right" label="调整后" placeholder="输入调整的价格"/>
        ...
        <footer class="supplementary-order-footer van-safe-area-bottom">
...
import { getSaleOrderDetailV10, finishOrder, getGoodsListV2 } from '@api/order.js'
...
const init = async () => {
    ...
    if (res.code === '200') {
        ...
        orderTotal.value = '¥' + ((orderInfo.value.orderInfo.totalAmount || 0) / 100).toFixed(2)
        handleChange()
        getSkutList()
    }
}
...
const getSkutList = async (flag) => {
    const param = {
        ...
        deviceCode: orderInfo.value.opendoorInfo.deviceId,
...
const onBlur = () => {
    auditPrice.value = (orderInfo.value.orderInfo.totalAmount || 0) - accMul(price.value || 0, 100)
...
const handleChange = () => {
    ...
    auditPrice.value = (orderInfo.value.orderInfo.totalAmount || 0) - skuPrice.value
    if (auditPrice.value > 0) {
        auditText.value = '需退款'
    } else {
        auditText.value = '需补扣'
...
const confirmEdit = async () => {
    Dialog.confirm({
        ...
        const param = {
            ...
            discAmount: orderInfo.value.orderInfo.disctAmount || 0, // 优惠券金额
            discernInfo: message.value, // 客服录入信息
            deductGoodsList: active.value === 0 ? checkSkuList.value.filter(el => el.quantity) : [],
            // goodsInfoList: [
            //     {
            //         quantity: 0, // 购买数量
            //         skuCode: 0, // 商品编码
            ...
            originType: 1 // 业务类型 1货柜 2电商
        }
        const res = await finishOrder(param)
        if (res.code === '200') {
            Toast('操作成功')
