<template>
    <div class="my-operation-page mine-content">
        ...
        <div class="operation-box">
            <div class="tip"><img :src="remark" /><span>说明：该列表为您合同尚未被绑定设备的合同设备1D</span></div>
                ...
                <div v-for="(item, index) in showOperationList" :key="'card' + index">
                    <div class="operation-card">
                        ...
                        <div class="operation-list" v-for="(point, index) in item.pointList" :key="'point' + index">
                            <van-checkbox v-model="point.checked" shape="square" checked-color="#FE7E41">{{ point.pointId }}</van-checkbox>
                        </div>
                    </div>
                </div>
            </van-pull-refresh>
        ...
        <div class="button-box-add">
            <van-button color="#FE7E41" block @click="addPoint" text="保存" />
        </div>
...
import { agentContractPointList, addAgentContract } from '@/api/subaccountnumber.js'
import { useStore } from 'vuex'
import remark from '@/assets/remark_icon.png'
const store = useStore()
const router = useRouter()
const route = useRoute()
// const props = defineProps(['merchantSubName', 'merchantSubType', 'realRight', 'merchantSubId', 'phoneMobile'])
...
console.log('asdasd', store.state.userInfo)

const getPointList = async () => {
    const params = {
        busiMerchantId: sessionStorage.getItem('merchantId'),
        merchantSubId: route.query.merchantSubId,
        customerMobile: sessionStorage.getItem('operatorPhone'),
        enableFlag: 1
    }
    const { code, data } = await agentContractPointList(params)
    if (Number(code) === 200) {
        Toast.clear()
        ...
        state.showOperationList = data.contractList
    }
}

const addPoint = async () => {
    let flag = false
    console.log('=state.showOperationList', showOperationList.value.length)
    const params = {
        busiMerchantId: sessionStorage.getItem('merchantId'),
        merchantSubId: route.query.merchantSubId,
        customerMobile: sessionStorage.getItem('operatorPhone'),
    ...
    showOperationList.value.forEach((item, index) => {
        const data = {
    ...
    const { code } = await addAgentContract(params)
    if (Number(code) === 200) {
        Toast('保存成功')
        router.push({
