        ...
        <div class="bill">
            <div class="bill-list">
                ...
                <div class="bill-item">
                    ...
                    <van-steps direction="vertical" :active="stepsActive" active-color="#FFC4A8" inactive-color="#CCCCCC" active-icon="stop-circle">
                        <van-step class="bill-item-main-detail" v-for="(item, index) in resultdata" :key="index">
                            <div class="bill-content">
                                <div class="text-left">
                ...
                <div class="container-pay" v-if="showButton">
                    <div class="pay-text">
                        <div class="total">总还款金额：<span class="total-icon">￥</span><span class="total-num">{{payAmount}}</span></div>
                    </div>
                    <div class="pay-button">
                        <van-button class="bottom-btn" round color="#FE7E41" text="30" @click="goFzPay">立即还款</van-button>
                    </div>
                </div>
            </div>
        </div>
...
import { confirmReveive, stageBillDetails, getWalletPrePay } from '@/api/container'
...
const stageBillDetailsFn = async () => {
    ...
    if (res.code === '200') {
        ...
        resultdata.value.map((el, index) => {
            if (el.currPeriods) {
                payAmount.value = el.amount
                period.value = el.periods
...
const goFzPay = async () => {
    ...
    const res = await getWalletPrePay({
        orderNo: props.orderNo,
    ...
    if (isApp) {
        if (isIos) {
            appBridge('checkout', { payurl: `${process.env.VUE_APP_LAYOUT_BASEURL_SYT}/${res.data.businessUniqueCode}/${res.data.prePayNo}?showNavbar=0&env=App&hideTitle=0&businessType=0&successUrl=${encodeURIComponent(location.origin + `/container/stag/pay/${props.orderNo}`)}&overtimeUrl=${encodeURIComponent(location.origin + `/container/stag/pay/${props.orderNo}`)}` })
        } else {
            appBridge('checkout', { payurl: `${process.env.VUE_APP_LAYOUT_BASEURL_SYT}/${res.data.businessUniqueCode}/${res.data.prePayNo}?showNavbar=0&env=App&hideTitle=0&businessType=0&successUrl=${encodeURIComponent(location.origin + `/container/stag/pay/${props.orderNo}`)}&backUrl=${encodeURIComponent(location.origin + `/container/stag/pay/${props.orderNo}`)}&overtimeUrl=${encodeURIComponent(location.origin + `/container/stag/pay/${props.orderNo}`)}` })
        }
    }
}
const goStagDetail = () => {
...
:deep(.van-step__icon--active::before) {
    ...
    width: 16px;
    height: 16px;
    background: url('~@/assets/pay-bill-icon.png') no-repeat;
